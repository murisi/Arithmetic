	...
	\begin{multicols}{2}
	  ...
	  \declaration{3.29}
		...
		\procedure{tue2008191254}
		  \objective
		    Choose two rational numbers $X>D>0$. The objective of the following instructions is to construct rational numbers $a,b,c,d$, a procedure $p(x,n)$ to show that $\lVert-\frac{1}{x^2}\rVert^2\le a^2$ when a complex number $x$ and a positive integer $n$ such that $\lVert x\rVert^2\ge X^2$ and $n>d$ are chosen, and a procedure $q(x,n,\delta)$ to show that $\Delta_{y=x}^{+\delta}\frac{1}{y}\equiv-\frac{1}{x^2}\err{\frac{b}{n}+c\{\delta\}}$ when in addition a complex number $\delta$ such that $\lVert\delta\rVert^2\le D^2$ is chosen.
		  \implementation
		    \begin{enumerate}
		      \item Let $a=\frac{1}{X^2}$.
		      \item Let $b=0$.
		      \item Let $c=\frac{1}{X^2(X-D)}$.
		      \item Let $p(x,n)$ be the following procedure:
		      \begin{enumerate}
