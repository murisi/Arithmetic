	  ...
	  \chapter{Polynomial Sign Changes}
	    \begin{multicols*}{2}
            ...
            \procedure{thu3001201111}
                \objective
                    Choose a polynomial $p$ and a rational number $X$. The objective of the following instructions is to construct a rational number $a$ and a procedure $q(y,z)$ to show that $\lVert\Lambda(p,z)\rVert\le a\lVert z-y\rVert$ and $\lVert\Lambda(p,y)\rVert\le a\lVert z-y\rVert$ when two rational numbers $y,z$ such that $\lVert y\rVert\le X$, $\lVert z\rVert\le X$, and $\Lambda(p,y)\Lambda(p,z)\le 0$ are chosen.
                \implementation
                    \begin{enumerate}
		    ...
		    \procedure{2.20}
			    \objective
				    Execute \procedurehr{2.19} and let $\langle s,q,g,h\rangle$ receive. Choose a rational number $X$. The objective of the following instructions is to construct a rational number $l$ and a procedure $u(c,d)$ to show that either $0<0$ or $\lvert \jsc_{s}(d)-\jsc_{s}(c)\rvert=[\sgn(\Lambda(s_{\lvert s\rvert-1},c))\ne\sgn(\Lambda(s_{\lvert s\rvert-1},d))]$, when rational numbers $c,d$ such that $\lvert c\rvert\le X$, $\lvert d\rvert\le X$, $\lvert d-c\rvert\le l$, $0\not\in\Lambda(s,c)$, and $0\not\in\Lambda(s,d)$ are chosen.
			    \implementation
				    \begin{enumerate}
					    ...
					    \begin{enumerate}
						    ...
						    \begin{enumerate}
							    ...
							    \item Verify that $\jsc_{s_{[0:i+1]}}(c)=\jsc_{s_{[0:i+1]}}(d)$.
							    \item If $\sgn(\Lambda(s_{i+1},c))=\sgn(\Lambda(s_{i+1},d))$, do the following:
							    \begin{enumerate}
								    \item Verify that $\jsc_{s_{[0:i+2]}}(c)=\jsc_{s_{[0:i+2]}}(d)$.
							    ...
							    \begin{enumerate}
								    ...
								    \item Verify that $\lvert \jsc_{s_{[0:i+2]}}(c)-\jsc_{s_{[0:i+2]}}(d)\rvert=1$.
							    ...
							    \begin{enumerate}
								    ...
								    \item Therefore verify that $1=\jsc_{s_{[0:i+3]}}(c)-\jsc_{s_{[0:i+1]}}(c)=\jsc_{s_{[0:i+3]}}(d)-\jsc_{s_{[0:i+1]}}(d)$.
								    \item Therefore verify that $\jsc_{s_{[0:i+1]}}(c)+1=\jsc_{s_{[0:i+3]}}(c)=\jsc_{s_{[0:i+3]}}(d)=\jsc_{s_{[0:i+1]}}(d)+1$.
						    ...
						    \begin{enumerate}
							    \item \textbf{Verify that $\jsc_s(c)=\jsc_s(d)$.}
						    \end{enumerate}
						    \item Otherwise do the following:
						    \begin{enumerate}
							    \item \textbf{Verify that $\lvert \jsc_s(d)-\jsc_s(c)\rvert=1$.}
						    \end{enumerate}
					    \end{enumerate}
