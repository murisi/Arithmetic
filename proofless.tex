	...
	\section{Body}\label{sec:body}
		...
		\subsection{Algorithm 55 (Symmetric matrix spectral algorithm initialization)}\label{sec:algorithm 55}
			...
			\begin{enumerate}
				...
				\begin{enumerate}
					...
					\item Otherwise, do the following:
					\begin{enumerate}
						\item \textbf{For $j=1$ to $j=m+1-k_i$, verify that $({K_i(c_i)}^TK_i(c_i))_{j,j}\ge \disc(A)$.}
		...
		\subsection{Algorithm 56 (Symmetric matrix spectral)}\label{sec:algorithm 56}
			...
			\begin{enumerate}
				...
				\item \textbf{Using \hyperref[sec:algorithm 55]{algorithm 55}, verify that all the diagonal entries of $K^TK$ are more than or equal to $\disc(A)$.}
