	  ...
	  \declaration{3.29}
		...
		\procedure{sat0308191134}
		  \objective
		    ...
		    \begin{enumerate}
		      \item A procedure $q_5(x,n,\delta)$ to show that $\lVert\frac{p_n(t_n(x+\delta))-p_n(t_n(x))}{\delta}-p'_n(t_n(x))t'_n(x)\rVert^2\le \frac{a_5}{n^2}+b_5\lVert\delta\rVert^2$ when two complex numbers $x,dx$ such that $R(x)$,$S(x)$,$P(t_n(x))$,$Q(t_n(x))$, $n>c_5$, and $\lVert\delta\rVert^2<d_5$ are chosen
		      \item A procedure $q_6(x,n)$ to show that $\lVert p'_n(t_n(x))t'_n(x)\rVert^2\le a_6$ when a complex number $x$ such that $Q(t_n(x))$, $S(x)$, and $n>b_6$ are chosen.
		    \end{enumerate}
		  \implementation
		    \begin{enumerate}
