		  ...
		  \objective
		    ...
		    \begin{enumerate}
		      \item A non-negative rational number $A$.
				...
				The objective of the following instructions is to construct the following:
				\begin{enumerate}
				  \item Non-negative rational numbers $a,b,c$.
				  \item A procedure $p(R,n)$ to show that $\int_r^Rf'_n(r)\delta_r\equiv f_n(R_{\lvert R\rvert-1})-f_n(R_0)\err{\frac{a}{n}+b\max(\{\Delta R\})}$ when an integer $n$ and a non-empty list of complex numbers $R$ such that $P(R_t)$ and $0<\lVert R_{t+1}-R_t\rVert^2<{d_1}^2$ for $t\in[0:\lvert R\rvert-1]$, $\int_r^R\{\delta_r\}\le A$, and $n>c$ are chosen.
				\end{enumerate}
		  \implementation
		    \begin{enumerate}
		      \item Let $a=a_1A$.
		  ...
		  \objective
		    ...
		    \begin{enumerate}
		      \item A non-negative rational number $A$.
				...
				\begin{enumerate}
				  \item Non-negative rational numbers $a,b,c$.
				  \item A procedure $p(R,n)$ to show that $\int_r^{g(R)}f_n(r)\delta_r\equiv\int_r^Rf_n(g_n(r))g'_n(r)\delta_r\err{\frac{a}{n}+b\max(\{\Delta R\})}$ when an integer $n$ and a non-empty list of complex numbers $R$ such that $P(R_t)$ and $0<\lVert R_{t+1}-R_t\rVert^2<{d_1}^2$ for $t\in[0:\lvert R\rvert-1]$, $\int_r^R\{\delta_r\}\le A$, and $n>c$ are chosen.
				\end{enumerate}
			\implementation
			  \begin{enumerate}
			    \item Let $a=a_1a_2A$.
