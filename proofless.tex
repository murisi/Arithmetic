...
\begin{document}
	...
	\section{Introduction}
		...
		Anyway, to see most easily what I am trying to do, see algorithms \hyperref[sec:algorithm 33]{33}, \hyperref[sec:algorithm 39]{39}, \hyperref[sec:algorithm 48]{48}, and \hyperref[sec:algorithm 50]{50}.
	\tableofcontents
	\section{Body}
		...
		\subsection{Algorithm 26}\label{sec:algorithm 26}
			\textbf{Choose four $m\times m$ matrices:
			\begin{enumerate}
				\item $A$ and $B$, whose entries are all rationals.
				\item $M$ and $N$, polynomial matrices that are the product of row/column operations, like $M$ and $N$ of \hyperref[sec:algorithm 5]{algorithm 5}.
				\item In such a way that $xI_m-B=M(xI_m-A)N$.
			\end{enumerate}} and do the following:
			\begin{enumerate}
				\item Let $N^{-1}$ be constructed from $N$ in a similar way to \hyperref[sec:algorithm 6]{algorithm 6}.
		...
		\subsection{Algorithm 35 (Bisection)}\label{sec:algorithm 35}
			...
			\end{enumerate}
		\subsection{Algorithm 36}\label{sec:algorithm 36}
			...
			\begin{enumerate}
				...
				\begin{enumerate}
					...
					\item Execute \hyperref[sec:algorithm 35]{algorithm 35} on the formal polynomial $f_i$, interval $(a_i, b_i)$, and target of $B$. Let $a_i$ and $b_i$ receive their updates.
				...
				\item \textbf{Verify that 0=1.}
			\end{enumerate}
		\subsection{Algorithm 37 (Sturm's algorithm initialization)}\label{sec:algorithm 37}
			...
			\begin{enumerate}
				...
				\begin{enumerate}
					\item Execute \hyperref[sec:algorithm 33]{algorithm 33} on $s_i$ and let $(G_i(x,y))$ be the result.
				...
				\end{enumerate}
			\end{enumerate}
		\subsection{Algorithm 38 (Change in number of sign changes verification)}\label{sec:algorithm 38}
			\begin{enumerate}
		...
		\subsection{Algorithm 41 (Sturm's sign change)}\label{sec:algorithm 41}
			\begin{enumerate}
				...
				\begin{enumerate}
					\item \textbf{Execute \hyperref[sec:algorithm 36]{algorithm 36} on the formal polynomial $s_m(x)$ along with the first $m+1$ recorded pairs.}
				\end{enumerate}
				...
				\end{enumerate}}
			\end{enumerate}
		\subsection{Algorithm 42}\label{sec:algorithm 42}
			...
			\begin{enumerate}
				\item Execute \hyperref[sec:algorithm 32]{algorithm 32} on matrix $A$ and polynomial $p$.
				\item Since the inner execution of \hyperref[sec:algorithm 1]{algorithm 1} at \hyperref[sec:algorithm 32]{algorithm 32}'s (5) was decreasing the degree of an entry on every iteration, verify for \hyperref[sec:algorithm 32]{algorithm 32}'s $g$ that $\deg(g)\le\deg(B_{m,m})-1$.
