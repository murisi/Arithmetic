...
\begin{document}
	...
	\section{Matrix Arithmetic}\label{sec:body}
		...
		\notation{10}
			Let us use the notation $A^{-1}$ as a shorthand for the result yielded by executing \hyperref[sec:procedure 6]{procedure 6} on $A$.
		...
		\procedure[Smith normal form construction]{10}
			...
			\implementation
				...
				\end{enumerate}
		\notation{12}
			Let us use the notation $\det(A)$ as a shorthand for the result yielded by executing \hyperref[sec:procedure 11]{procedure 11} on $A$.
		...
		\procedure[Compound matrix calculation]{16}
			...
			\implementation
				...
				\end{enumerate}
		\notation{13}
			We will use the notation $C_k(A)$ to refer to the result yielded by executing \hyperref[sec:procedure 16]{procedure 16} on the matrix $A$ and integer $k$.
		\notation{14}
		...
		\procedure[Determinant equals product of diagonal entries verification]{23}
			\objective
				Choose a $\mathcal{M}_{m,m}(\mathbb{Q}[x])$, $A$. Let $D$ be a copy of $A$. Execute \hyperref[sec:procedure 10]{procedure 10} on $D$. The objective of the following instructions is to show that $\det(A)$ is the product of the diagonal entries of $D$.
			\implementation
				\begin{enumerate}
					\item Execute \hyperref[sec:procedure 21]{procedure 21} on $A$ and let $\langle M,D,,N\rangle$ receive.
		...
		\procedure[Transpose calculation]{24}
			...
			\implementation
				...
				\end{enumerate}
		\notation{15}
			Let us use the notation $A^T$ for the result yielded by executing \hyperref[sec:procedure 24]{procedure 24} on $A$.
		...
		\procedure{36}
			...
			\implementation
				\begin{enumerate}
					...
					\item Therefore using \hyperref[sec:procedure 23]{procedure 23}, verify that $\det(B)=\det(xI-A)$.
			...
			\implementation
				\begin{enumerate}
					...
					\begin{enumerate}
						...
						\item \textbf{Also verify that ${A_i}^TA_i=\bdiag(({A_i}^TA_i)_{[1:i+1],[1:i+1]},({A_i}^TA_i)_{[i+1:n+1],[i+1:n+1]})$.}
					\end{enumerate}
				\end{enumerate}
		\procedure{80}
			\objective
