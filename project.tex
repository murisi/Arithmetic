...
\begin{document}
	...
	\section{Rational Arithmetic}
		...
		\procedure[Difference of powers]{1.0}
			\objective
			...
			\implementation
				\begin{enumerate}
					...
					\begin{enumerate}
						...
						\begin{enumerate}
							\item Execute \procedurehr{1.0} on $g$ and let the tuple $\langle G\rangle$ receive the result.
		...
		\procedure[Bisection]{1.2}
			...
			\implementation
				\begin{enumerate}
					\item Execute \procedurehr{1.0} on $f$ and let the tuple $\langle G\rangle$ receive the result.
			...
			\implementation
				\begin{enumerate}
					...
					\begin{enumerate}
						...
						\item Execute \procedurehr{1.2} on the formal polynomial $f$, interval $(a_n, b_n)$, and target of $B$. Let the tuple $\langle d\rangle$ receive the result.
						\item Verify that $\lvert f(d)\rvert<B$.
						\item Execute \procedurehr{1.0} on the formal polynomial $f$ and let the tuple $\langle G\rangle$ receive the result.
						\item Let $x$ be an indeterminate.
						...
						\begin{enumerate}
							...
							\end{enumerate}
						\end{enumerate}
						\item Execute \procedurehr{1.3} on $h$ and $a_{n-1}<b_{n-1}\le a_{n-2}<b_{n-2}\le\cdots\le a_1<b_1\le a_0<b_0$.
					\end{enumerate}
		...
		\procedure[Sturm's procedure initialization]{1.4}
			\objective
			...
			\implementation
				\begin{enumerate}
					...
					\begin{enumerate}
						...
						\item Execute \procedurehr{1.4} on $s_{[0:m]}$ and $q_{[1:m-1]}$ and let the tuple $\langle,,g,h\rangle$ receive.
		...
		\procedure[Change in number of sign changes verification]{1.5}
			\objective
				Execute \procedurehr{1.4} and let $\langle s,q,g,h\rangle$ receive. Execute \procedurehr{1.0} on $s$ and let $\langle G\rangle$ receive the result. Choose $\mathbb{Q}$s $c$ and $d$ in such a way that:
				\begin{enumerate}
			...
			\implementation
				\begin{enumerate}
					...
					\begin{enumerate}
						...
						\begin{enumerate}
							\item Execute \hyperref[sec:procedure 1.5 auxilliary procedure]{procedure 1.5 auxilliary procedure} on $i$.
				...
				\end{enumerate}
			\subsubsection*{Auxilliary Procedure}\label{sec:procedure 1.5 auxilliary procedure}
				\paragraph{Objective}
				...
				\paragraph{Implementation}
					\begin{enumerate}
						...
						\end{enumerate}
					\end{enumerate}
		\procedure[Cauchy's positive verification]{1.6}
			\objective
			...
			\implementation
				\begin{enumerate}
					...
					\end{enumerate}
				\end{enumerate}
		\procedure[Cauchy's alternation verification]{1.7}
			\objective
			...
			\implementation
				\begin{enumerate}
					...
					\item Execute \procedurehr{1.6} on $q$ and $-k$.
				...
				\end{enumerate}
		\procedure[Range subdivision]{1.8}
			\objective
					...
					\begin{enumerate}
						...
						\begin{enumerate}
							...
							\begin{enumerate}
								...
								\item Execute \procedurehr{1.1} on $s_i$ and a sorted $e_i$.
								\item \textbf{Abort procedure.}
							\end{enumerate}
						\end{enumerate}
						\item Otherwise, do the following:
		...
		\procedure[Sturm's sign change]{1.9}
			\objective
				Execute \procedurehr{1.4} and let $\langle s,q,g,h\rangle$ receive. Let $m=\lvert s\rvert-1$. The objective of the following instructions is to either show that $0<0$ or to construct two lists of rational numbers $c,d$ such that $c_1<d_1\le c_2<d_2\le\cdots\le c_m<d_m$ and $\sgn(s_m(c_i))=-\sgn(s_m(d_i))$ for $i=1$ to $i=m$.
			\implementation
				\begin{enumerate}
					\item Let $U=1+\max_{i=0}^m\left(1+\max_{j=1}^i\lvert\frac{x^{i-j}\circ s_i}{x^i\circ s_i}\rvert\right)$
					\item Using \procedurehr{1.6}, verify that $J(U)=0$.
					\item Using \procedurehr{1.7}, verify that $J(-U)=m$.
					\item Execute \procedurehr{1.0} on $s$ and let $\langle G\rangle$ receive the result.
					...
					\item Execute \procedurehr{1.8} on $s$ with endpoints $-U,U$ and a step size of $l$ and let $\langle e\rangle$ receive the result.
					\item Let $c=d=\langle\rangle$.
					\item For $i=1$ to $i=\lvert e\rvert-1$:
					\begin{enumerate}
						\item Execute \procedurehr{1.5} on the tuple $\langle e_i,e_{i+1}\rangle$.
						\item If $J_m(c)\ne J_m(d)$, then do the following:
						\begin{enumerate}
							\item Append $e_i$ to $c$.
							\item Append $e_{i+1}$ to $d$.
							\item Cognizant of \procedurehr{1.5}, verify that $\lvert J_m(d)-J_m(c)\rvert=1$.
	...
	\section{Matrix Arithmetic}
				...
				\begin{enumerate}
					...
					\item Now do the following:
						...
						\begin{enumerate}
							...
							\item Apply \procedurehr{2.0} on this submatrix.
					...
					\item Apply \procedurehr{2.1} on the submatrix $A_{[2:m+1],[2:n+1]}$.
					...
					\item \textbf{Yield the tuple $\langle A\rangle$.}
				\end{enumerate}
		\procedure[Associativity verification]{2.2}
			\objective
		...
		\procedure[Row and column operation recording]{2.3}
			\objective
			...
			\implementation
				...
				\end{enumerate}
		\procedure[Multiplication by identity]{2.4}
			\objective
			...
			\implementation
				...
				\end{enumerate}
		\notation{2.5}
			Let us use the notation $M_*$ as a shorthand for "$M_1M_2\cdots M_{\lvert M\rvert}$".
		\notation{2.6}
			Let us use the notation $A^{-1}$ as a shorthand for the result yielded by executing \procedurehr{2.5} on $A$.
		\procedure[Matrix list inversion]{2.5}
			\objective
			...
			\implementation
				\begin{enumerate}
					...
					\item Therefore using \procedurehr{2.2} and \procedurehr{2.4}, verify that $A_*{A^{-1}}_*$
					\begin{enumerate}
		...
		\procedure{2.6}
			...
			\implementation
				\begin{enumerate}
					...
					\item \textbf{Therefore using \procedurehr{2.5}, verify that ${A^{-1}}_*A_*={A^{-1}}_*{(A^{-1})^{-1}}_*=I_m$.}
				\end{enumerate}
		\procedure{2.7}
			\objective
			...
			\implementation
				\begin{enumerate}
					...
					\begin{enumerate}
						...
						\item Let $\langle M,N\rangle$ receive the results of executing \procedurehr{2.3} on the pair $\langle m,n\rangle$ and the following procedure:
						\begin{enumerate}
							\item Execute \procedurehr{2.0} on the submatrix of $A'$ formed by selecting row $1$ and columns $1$ and $j$ as if there were nothing in between.
						\end{enumerate}
				...
				\end{enumerate}
		\procedure[Block matrix multiplication]{2.8}
			\objective
			...
			\implementation
				...
				\end{enumerate}
		\procedure[Smith normal form construction]{2.9}
			\objective
			...
			\implementation
				\begin{enumerate}
					\item Apply \procedurehr{2.1} on matrix $A$.
					...
					\begin{enumerate}
						...
						\item Let $\langle M,N\rangle$ receive the results of executing \procedurehr{2.3} on the pair $\langle m,n\rangle$ and the following procedure:
						\begin{enumerate}
							\item Apply \procedurehr{2.7} on the submatrix of $A'$ containing rows $j$ to $m$ and columns $j$ to $n$, and let $\langle u\rangle$ receive.
						\end{enumerate}
		...
		\notation{2.8}
			Let us use the notation $\det(A)$ as a shorthand for the result yielded by executing \procedurehr{2.10} on $A$.
		\procedure[Determinant calculation]{2.10}
			\objective
			...
			\implementation
				\begin{enumerate}
					...
					\end{enumerate}
				\end{enumerate}
		\procedure[Multilinearity verification]{2.11}
			\objective
			...
			\implementation
				\begin{enumerate}
					...
					\begin{enumerate}
						...
						\begin{enumerate}
							\item Execute \procedurehr{2.11} on $\langle p,B_{[1:r]^\frown[r+1:m+1],1},C_{[1:r]^\frown[r+1:m+1],1},i-1,\allowbreak A_{[1:r]^\frown[r+1:m+1],[2:m+1]}\rangle$.
		...
		\procedure[Alternation verification]{2.12}
			\objective
			...
			\implementation
				\begin{enumerate}
					...
					\begin{enumerate}
						...
						\item For $r=1$ to $r=m$, do the following:
						\begin{enumerate}
							\item Execute \procedurehr{2.12} on $\langle i-1,\allowbreak A_{[1:r]^\frown[r+1:m+1],[2:m+1]}\rangle$.
			...
			\implementation
				\begin{enumerate}
					...
					\begin{enumerate}
						...
						\item For $r=1$ to $r=m$, do the following:
						\begin{enumerate}
							\item Execute \procedurehr{2.13} on $\langle i-1,\allowbreak A_{[1:r]^\frown[r+1:m+1],[2:m+1]}\rangle$.
			...
			\implementation
				\begin{enumerate}
					...
					\begin{enumerate}
						...
						\item Using \procedurehr{2.12}, verify that $\det(A_k)=-\det(A_{k-1})$.
					\end{enumerate}
		...
		\procedure[Compound matrix calculation]{2.15}
			\objective
			...
			\implementation
				...
				\end{enumerate}
		\notation{2.9}
			We will use the notation $C_k(A)$ to refer to the result yielded by executing \procedurehr{2.15} on the matrix $A$ and integer $k$.
		\notation{2.10}
			We will use the notation $A_{\ul{I},\ul{J}}$ to refer to the entry of $A$ with row label $I$ and column label $J$.
		\procedure[Compound matrix of identity calculation]{2.16}
			\objective
				...
				\begin{enumerate}
					...
					\begin{enumerate}
						...
						\begin{enumerate}
							...
							\item \textbf{Therefore using \procedurehr{2.10}, verify that $(C_k(I_m))_{\ul{I},\ul{J}}=\det((I_m)_{I,J})=\det(I_k)=1$.}
						\end{enumerate}
						\item Otherwise, do the following:
						\begin{enumerate}
							\item Verify that $I\ne J$.
							...
							\item \textbf{Therefore using \procedurehr{2.10}, verify that $(C_k(I_m))_{\ul{I},\ul{J}}=\det((I_m)_{I,J})=0$.}
						\end{enumerate}
					\end{enumerate}
				...
				\begin{enumerate}
					...
					\begin{enumerate}
						...
						\begin{enumerate}
							...
							\item For each column label $J$: Using \procedurehr{2.11}, verify that ${C_k(AB)}_{\ul{I},\ul{J}}=\det((AB)_{I,J})=\det(B_{I,J})+p*\det(B_{I',J})$.
							\item If $j\in I$, then do the following:
							\begin{enumerate}
								\item Verify that the sequence $I'$ contains two $j$s.
								\item For each column label $J$: Using \procedurehr{2.13} verify that $\det(B_{I',J})=0$.
							...
							\begin{enumerate}
								...
								\item For each column label $J$: Using \procedurehr{2.14}, verify that $\det(B_{I',J})=(-1)^l\det(B_{I'',J})$.
			...
			\implementation
				\begin{enumerate}
					...
					\begin{enumerate}
						...
						\begin{enumerate}
							\item Using \procedurehr{2.15}, verify that every element of $I$ is less than or equal to $\min(m,n)$.
							\item Let $A_0=A$.
							\item For $i=1$ to $i=k$: Let $A_i$ equal $A_{i-1}$ but with position $(I_i,I_i)$ set to $1$.
							\item For each column label $J$: Repeatedly using \procedurehr{2.11}, verify that ${C_k(AB)}_{I,J}$
							\begin{enumerate}
								\item $=\det((AB)_{I,J})$
								\item $=\det((A_0B)_{I,J})$
						...
						\begin{enumerate}
							...
							\item Therefore using \procedurehr{2.10}, for each column label $J$: verify that ${C_k(AB)}_{I,J}=\det((AB)_{I,J})=0$.
		...
		\procedure{2.19}
			...
			\implementation
				\begin{enumerate}
					\item Execute \procedurehr{2.17} on matrices $A$ and $I_m$. Let $D$ be the matrix constructed.
					\item Using \procedurehr{2.16}, verify that $C_k(A)=C_k(AI_m)=DC_k(I_m)=DI_{\binom{m}{k}}=D$.
					\item Execute \procedurehr{2.17} on matrices $A$ and $B$. Let $D'$ be the matrix constructed.
			...
			\textbf{Using \procedurehr{2.18}, make a procedure similar to the above but that only instead allows for a diagonal matrix of $\mathbb{Q}[x]$s, $A$, to be chosen.}
		\procedure{2.20}
			\objective
			...
			\implementation
				\begin{enumerate}
					...
					\item Let $\langle M,N\rangle$ receive the results of executing \procedurehr{2.3} on the pair $\langle m,n\rangle$ and the following procedure:
						\begin{enumerate}
							\item Execute \procedurehr{2.9} on the matrix $D$ and let $\langle v\rangle$ receive.
						\end{enumerate}
				...
				\end{enumerate}
		\procedure[Compound matrix of matrix product calculation]{2.21}
			\objective
			...
			\implementation
				\begin{enumerate}
					\item Execute \procedurehr{2.20} on $A$ and let $\langle M,D,,N\rangle$ receive.
					\item Using repeated applications of \procedurehr{2.19}, verify that $C_k(AB)$
					\begin{enumerate}
					...
					\end{enumerate}
				\end{enumerate}
		\procedure[Determinant equals product of diagonal entries verification]{2.22}
			\objective
				Choose a $\mathcal{M}_{m,m}(\mathbb{Q}[x])$, $A$. Let $D$ be a copy of $A$. Execute \procedurehr{2.9} on $D$. The objective of the following instructions is to show that $\det(A)$ is the product of the diagonal entries of $D$.
			\implementation
				\begin{enumerate}
					\item Execute \procedurehr{2.20} on $A$ and let $\langle M,D,,N\rangle$ receive.
					\item Using \procedurehr{2.10} and \procedurehr{2.21}, verify that $\det(A)$
					\begin{enumerate}
						\item $=C_m(A)$
						...
						\item $=\det(D)$.
					\end{enumerate}
					\item \textbf{Using \procedurehr{2.10}, verify that $\det(D)$ is the product of the diagonal entries of $D$.}
				\end{enumerate}
		\procedure[Transpose calculation]{2.23}
			\objective
			...
			\implementation
				...
				\end{enumerate}
		\notation{2.11}
			Let us use the notation $A^T$ for the result yielded by executing \procedurehr{2.23} on $A$.
		\procedure[Transpose of product verification]{2.24}
			\objective
			...
			\implementation
				\begin{enumerate}
					...
					\begin{enumerate}
						\item For $j=1$ to $j=m$:
						\begin{enumerate}
							\item Using \procedurehr{2.23}, verify that $(B^TA^T)_{i,j}=\sum_{l=0}^n B_{l,i}A_{j,l}=\sum_{l=0}^n A_{j,l}B_{l,i}=(AB)_{j,i}=((AB)^T)_{i,j}$.
						\end{enumerate}
					\end{enumerate}
					\item \textbf{Therefore verify that $B^TA^T=(AB)^T$.}
				\end{enumerate}
		\procedure[Determinant of transpose verification]{2.25}
			\objective
				Choose a $\mathcal{M}_{m,m}(\mathbb{Q}[x])$, $A$. The objective of the following instructions is to show that $\det(A^T)=\det(A)$.
			\implementation
				\begin{enumerate}
					\item Execute \procedurehr{2.20} on $A$ and let $\langle M,D,,N\rangle$ receive.
					\item Therefore using procedures \procedurehr{2.22} and \procedurehr{2.24}, verify that $\det(A^T)$
					\begin{enumerate}
					...
					\end{enumerate}
				\end{enumerate}
		\procedure[Compound matrix of transpose verification]{2.26}
			\objective
			...
			\implementation
				\begin{enumerate}
					...
					\begin{enumerate}
						...
						\begin{enumerate}
							\item Using \procedurehr{2.25}, verify that $(C_k(A^T))_{\ul{I},\ul{J}}=\det((A^T)_{I,J})=\det(A_{J,I})=(C_k(A))_{\ul{J},\ul{I}}$.
						\end{enumerate}
					\end{enumerate}
					\item \textbf{Therefore verify that $(C_k(A))^T=(C_k(A^T))$.}
				\end{enumerate}
		\procedure[Linear system solution construction]{2.27}
			\objective
				Choose a $\mathcal{M}_{m,n}(\mathbb{Q})$, $A$, and a $\mathcal{M}_{m,p}(\mathbb{Q})$, $B$. Execute \procedurehr{2.20} on $A$ and let $\langle M,D,,N\rangle$ receive the result. If the indices of the rows of $D$ that are entirely zero are also the indices of the rows of $MB$ that are entirely zero, then the objective of the following instructions is to construct a $\mathcal{M}_{n,p}(\mathbb{Q})$ $E$ such that $AE=B$.
			\implementation
				\begin{enumerate}
					...
					\item Let $E$ be $NC$.
					\item \textbf{Therefore using \procedurehr{2.5}, verify that $AE=M^{-1}DN^{-1}E=M^{-1}DN^{-1}NC=M^{-1}DI_nC=M^{-1}DC=M^{-1}MB=I_mB=B$.}
					\item \textbf{Yield the tuple $\langle E\rangle$.}
				\end{enumerate}
		\notation{2.12}
			The notation $A\backslash B$ shall be used to refer to the result yielded by executing \procedurehr{2.27} on matrices $A$ and $B$.
		\notation{2.13}
		...
		\procedure{2.28}
			\objective
				Choose a $\mathcal{M}_{m,n}(\mathbb{Q})$, $A$, a $\mathcal{M}_{n,p}(\mathbb{Q})$, $E$, and a $\mathcal{M}_{m,p}(\mathbb{Q})$, $B$ such that $AE=B$. Execute \procedurehr{2.20} on $A$ and let $\langle M,D,,N\rangle$ receive the result. If the indices of the rows of $D$ that are entirely zero are not also the indices of the rows of $M_*B$ that are entirely zero, then the objective of the following instructions is to show that $0\ne 0$.
			\implementation
				\begin{enumerate}
		...
		\procedure{2.29}
			...
			\implementation
				\begin{enumerate}
					\item Execute \procedurehr{2.20} on $B$ and let $\langle M,D,,N\rangle$ receive the result.
					\item Verify that $B={M^{-1}}_*D{N^{-1}}_*$.
					\item If $D$ has a zero on its diagonal, then do the following:
					\begin{enumerate}
						\item Using \procedurehr{2.22}, verify that $\det(I_m)=\det(AB)=\det(A)\det(B)=\det(A)\det(D)=\det(A)*0=0$.
						\item Using \procedurehr{2.10}, verify that $\det(I_m)=1^m=1$.
		...
		\procedure{2.31}
			...
			\implementation
				\begin{enumerate}
					...
					\item Execute \procedurehr{2.30} on $\langle M,B\rangle$ and let $\langle Q_1,R_1\rangle$ receive.
					\item Verify that $M=(xI_m-B)Q_1+R_1$.
					\item Execute \procedurehr{2.30} on $\langle N^{-1},A\rangle$ and let $\langle Q_2,R_2\rangle$ receive.
					...
					\item Execute \procedurehr{2.30} on $\langle M^{-1},A\rangle$ and let $\langle Q_3,R_3\rangle$ receive.
					...
					\item \textbf{Therefore using \procedurehr{2.29}, verify that $R_3R_1=I_m$.}
		...
		\procedure{2.33}
			...
			\implementation
				\begin{enumerate}
					\item Let $A$ be our working matrix.
					\item Use \procedurehr{2.32} to negate the $i^{th}$ row and swap it with the $j^{th}$ row.
					\item Use \procedurehr{2.32} to negate the $i^{th}$ column and swap it with the $j^{th}$ column.
		...
		\procedure{2.35}
			\objective
				Choose a $\mathcal{M}_{m,m}(\mathbb{Q})$, $A$. Execute \procedurehr{2.9} on the polynomial matrix $xI-A$ and let $\langle B\rangle$ be the result. The objective of the following instructions is to show that either none of the diagonal entries of $B$ are equal to zero, or $1=0$.
			\implementation
				\begin{enumerate}
					\item Using \procedurehr{2.10}, verify that $\det(xI-A)$ is a monic polynomial of degree $m$.
					\item Therefore using \procedurehr{2.22}, verify that $\det(B)=\det(xI-A)$.
					...
					\begin{enumerate}
						\item Using \procedurehr{2.10}, verify that $\det(B)=B_{1,1}B_{2,2}\cdots B_{m,m}=0$.
		...
		\procedure[Block diagonal construction]{2.36}
			\objective
			...
			\implementation
				...
				\end{enumerate}
		\notation{2.17}
			Let us use the notation $\bdiag(C)$ as a shorthand for the result yielded by executing \procedurehr{2.36} on $C$.
		\procedure{2.37}
			\objective
				Choose a positive integer $m$ and an $\mathcal{M}_{m,m}(\mathbb{Q})$, $A$. Execute \procedurehr{2.20} on the polynomial matrix $xI_m-A$ and let $\langle ,B,v,\rangle$ be the result. The objective of the following instructions is to either show that $0<0$ or to construct an integer $a$ such that $\sum_{i=a}^m\deg(B_{i,i})=m$, $\deg(B_{i,i})>0$ for $i=a$ to $i=m$, and $\deg(B_{i,i})=0$ for $i=1$ to $i=a-1$.
			\implementation
				\begin{enumerate}
					\item Execute \procedurehr{2.35} on $A$.
					...
					\end{enumerate}
				\end{enumerate}
		\procedure[Rational canonical form construction]{2.38}
			\objective
			...
			\implementation
				...
				\end{enumerate}
		\notation{2.18}
			Let us use $\rcan(p)$ as a shorthand for the result yielded by executing \procedurehr{2.38} on $p$.
		\procedure{2.39}
			\objective
			...
			\implementation
				\begin{enumerate}
					...
					\item For $i=2$ going up to $i=k$, negate row $i-1$ and exchange it with row $i$ using \procedurehr{2.32}.
		...
		\procedure{2.40}
			\objective
				Choose a positive integer $m$ and an $\mathcal{M}_{m,m}(\mathbb{Q})$, $A$. Execute \procedurehr{2.3} on the polynomial matrix $xI_m-A$ and let $\langle,B,,\rangle$ receive the result. Execute \procedurehr{2.37} on $A$ and let $\langle a\rangle$ receive the result. Let $E_i=\rcan(\mon(B_{a-1+i,a-1+i}))$ for $i=1$ to $i=m+1-a$. The objective of the following instructions is to first show that $\cols(\bdiag(E))=m$, and second to apply a sequence of elementary operations on $xI_m-\bdiag(E)$ to obtain the matrix $B$.
			\implementation
				\begin{enumerate}
					\item Verify that the diagonal of $B$ comprises $x-1$ rationals followed by $B_{a,a},B_{a+1,a+1},\cdots,B_{m,m}$.
					\item \textbf{Using \procedurehr{2.39}, verify that $\cols(\bdiag(E))=\sum_{i=1}^{\lvert E\rvert}\cols(E_i)=\sum_{i=1}^{\lvert E\rvert}\cols(\rcan(\mon(B_{a-1+i,a-1+i})))=\sum_{i=1}^{\lvert E\rvert}\deg(\mon(B_{a-1+i,a-1+i}))=\sum_{i=1}^{m+1-a}\deg(B_{a-1+i,a-1+i})=\sum_{i=a}^m\deg(B_{i,i})=m$.}
					\item Let $F=xI_m-\bdiag(E)$.
					\item Now for $i=1$ to $i=\lvert E\rvert$:
					\begin{enumerate}
						\item Let $j=1+\sum_{r=1}^{i-1}\cols(E_r)$.
						\item Let $k=j+\cols(E_i)$.
						\item Apply \procedurehr{2.39} on the tuple $\langle\mon(B_{a-1+i,a-1+i}),F_{[j:k],[j:k]}\rangle$.
					\end{enumerate}
					...
					\item Rearrange the diagonal of $F$ so that $\mon(B_{i,i})$ is at the $i^{th}$ position on the diagonal for $i=a$ to $i=m$ by doing pairwise swaps. In general, swap the $i^{th}$ and $j^{th}$ diagonal entries using \procedurehr{2.33}.
					\item For $i=1$ to $i=m-1$, do the following:
					\begin{enumerate}
						\item Let $k=\frac{x^{\deg(B_{i,i})}\circ B_{i,i}}{x^{\deg(F_{i,i})}\circ F_{i,i}}$.
						\item Scale $B_{i,i}$ by $k$ and $B_{i+1,i+1}$ by $\frac{1}{k}$ using \procedurehr{2.34}.
		...
		\procedure{2.41}
			\objective
				Choose a $\mathcal{M}_{m,m}(\mathbb{Q})$, $A$. Execute \procedurehr{2.37} on $A$ and let $\langle a\rangle$ receive the result. Let $E_i=\rcan(\mon(B_{a-1+i,a-1+i}))$ for $i=1$ to $i=m+1-a$. The objective of the following instructions is to either show that $0=1$ or to construct $\mathcal{M}_{m,m}(\mathbb{Q})$s $R,T$ such that $A=R\bdiag(E)T$, $RT=I_m$, and $TR=I_m$.
			\implementation
				\begin{enumerate}
					\item Execute \procedurehr{2.20} on the polynomial matrix $xI_m-A$ and let $\langle P,B,,Q\rangle$ be the result.
					\item Verify that $P_*(xI_m-A)Q_*=B$.
					\item Verify that $xI_m-A={P^{-1}}_*B{Q^{-1}}_*$.
					\item Let $Z$ be a variant of \procedurehr{2.20} where every occurence of \procedurehr{2.9} in its instructions is replaced with \procedurehr{2.40}, and where every mention of $v$ is ignored.
					...
					\item Execute \procedurehr{2.31} on the matrices $\langle A,{P}^{-1}M,\bdiag(E),N{Q}^{-1}\rangle$. Let the tuple $\langle R,T\rangle$ be the result.
		...
		\procedure{2.43}
			...
			\implementation
				\begin{enumerate}
					\item For $i=0$ up to $i=t$, by repeated applications of \procedurehr{2.8}, verify that $\bdiag(A)^i$ evaluates to $\bdiag(A^i)$ (where the exponentiation is done element-wise).
		...
		\procedure{2.44}
			\objective
				Choose a $\mathcal{M}_{m,m}(\mathbb{Q})$, $A$, and a $\mathbb{Q}[x]$, $r$. Execute \procedurehr{2.41} on the matrix $A$ and let the tuple $\langle R_1,E,R_3\rangle$ receive the result. The objective of the following instructions is to show that $r(A)=R_1\bdiag(r(E))R_3$ (where $r$ is applied element-wise).
			\implementation
				\begin{enumerate}
					\item Verify that $R_3R_1=I_m$.
					\item Using \procedurehr{2.42}, verify that $r(A)=r(R_1\bdiag(E)R_3)=R_1r(\bdiag(E))R_3$.
					\item Using \procedurehr{2.43}, verify that $r(\bdiag(E))=\bdiag(r(E))$ (where $r$ is applied element-wise).
		...
		\procedure{2.46}
			...
			\implementation
				\begin{enumerate}
					\item Execute \procedurehr{2.20} on the polynomial matrix $xI_m-A$ and let the tuple $\langle,B,,\rangle$ receive the result.
					\item Execute \procedurehr{2.35} on $A$.
					\item Verify that $B_{m,m}\ne 0$.
					\item Yield $\langle B_{m,m}\rangle$.
				\end{enumerate}
		\notation{2.22}
			Let us use the notation $\last_A$ as a shorthand for the result of executing \procedurehr{2.46} on $A$.
		\procedure{2.47}
			\objective
			...
			\implementation
				\begin{enumerate}
					\item Execute \procedurehr{2.20} on the matrix $A$ and let the tuple $\langle M,B,v,N\rangle$ receive the result.
					\item Execute \procedurehr{2.37} on $A$ and let $\langle a\rangle$ receive.
					\item Execute \procedurehr{2.41} on $A$ and let $\langle R,E,T\rangle$ receive.
					...
					\begin{enumerate}
						...
						\item Therefore using \procedurehr{2.45} verify that $\last_A(E_j)=B_{m,m}(E_j)=B_{a-1+j,a-1+j}(\rcan(\mon(B_{a-1+j,a-1+j})))\cdot v_{a+j}(E_j)v_{a+j+1}(E_j)\cdots v_m(E_j)=0_{k\times k}v_{a+j}(E_j)v_{a+j+1}(E_j)\cdots v_m(E_j)=0_{k\times k}$.
					\end{enumerate}
					\item \textbf{Therefore using \procedurehr{2.44} verify that $\last_A(A)=R\bdiag(\last_A(E))T=R\bdiag(B_{m,m}(E))T=R0_{m\times m}T=0_{m\times m}$.}
				\end{enumerate}
		\procedure{2.48}
			\objective
		...
		\procedure{2.50}
			...
			\implementation
				\begin{enumerate}
					...
					\item Execute \procedurehr{2.20} on $F$ and let $\langle M,D,,N\rangle$ receive the result.
					...
					\item Therefore using \procedurehr{2.45}, verify that $g(A)=N_{1,1}(A)p(A)+N_{2,1}(A)\last_A(A)=N_{1,1}(A)0_{m\times m}+N_{2,1}(A)0_{m\times m}=0_{m\times m}$.
					\item Execute \procedurehr{2.41} on the matrix $A$ and let the tuple $\langle R_1,E,R_3\rangle$ receive the result.
					\item Using \procedurehr{2.44}, and \procedurehr{2.41}, verify that $\bdiag(g(E))=I_m\bdiag(g(E))I_m=R_3R_1\bdiag(g(E))R_3R_1=R_3g(A)R_1=R_30_{m\times m}R_1=0_{m\times m}$.
					\item Let $G=\rcan(\mon(\last_A))$.
					\item Verify that $g(G)=g(E_{\lvert E\rvert})=\bdiag(g(E))_{[m-u+1:m+1],[m-u+1:m+1]}=0_{u\times u}$.
					\item If $w<u$, then:
					\begin{enumerate}
						\item Using \procedurehr{2.48}, verify that $g(G)\ne 0_{u\times u}$.
						\item \textbf{Abort procedure.}
					\end{enumerate}
					\item Otherwise, do the following:
					\begin{enumerate}
						\item Verify that $w=u$.
						\item Using \procedurehr{2.49}, verify that $g=g_0\last_A$.
		...
		\procedure{2.51}
			...
			\implementation
				...
				\end{enumerate}
		\notation{2.23}
			Let us use the notation $\pows(A)$ as a shorthand for the result yielded by executing \procedurehr{2.51} on $A$.
		\procedure{2.52}
			\objective
		...
		\procedure{2.53}
			...
			\implementation
				\begin{enumerate}
					\item Execute \procedurehr{2.50} on $A$ and $p$ and let $f$ receive.
		...
		\procedure{2.54}
			\objective
				Choose a $\mathcal{M}_{m,m}(\mathbb{Q})$, $A$. Execute \procedurehr{2.20} on $\pows(A)$ and let the tuple $\langle M,D,,N\rangle$ receive the result. Let $t=\cols(\pows(A))$. The objective of the following instructions is to show that either $0<0$ or to show that ${C_t(D)}={C_t(D)}_{1,1}e_1\ne 0$.
			\implementation
				\begin{enumerate}
					\item Execute \procedurehr{2.20} on $\pows(A)$ and let the tuple $\langle M,D,,N\rangle$ receive the result.
					\item Verify that $M_*\pows(A)N_*=D$.
					\item Using \procedurehr{2.5}, verify that $M^{-1}MFN=I_{m^2}FN=FN=M^{-1}D$.
					...
					\begin{enumerate}
						...
						\item Execute \procedurehr{2.52} on ${N^{-1}}_*$ and $N_*$.
						\item Therefore verify that $p\ne 0$.
						\item Execute \procedurehr{2.53} on $A$ and $p$.
						\item \textbf{Abort procedure.}
					\end{enumerate}
					\item Otherwise, do the following:
					\begin{enumerate}
						\item Execute \procedurehr{2.18} on $D,I_t,t$ and let $E$ receive.
		...
		\procedure{2.56}
			...
			\implementation
				\begin{enumerate}
					\item Using \procedurehr{2.26} and \procedurehr{2.55}, verify that $C_t(\pows(A)^T\pows(A))=C_t(\pows(A)^T)C_t(\pows(A))={C_t(\pows(A))}^TC_t(\pows(A))=\lVert C_t(\pows(A))\rVert^2>0$.
				...
				\end{enumerate}
		\notation{2.27}
			Let us use the notation $\sel_A$ as a shorthand for the result yielded by executing \procedurehr{2.56} on $A$.
		\notation{2.28}
		...
		\procedure{2.58}
			...
			\implementation
				\begin{enumerate}
					\item Using \procedurehr{2.57} and \procedurehr{2.56}, verify that $\tr(u(A)\sel_A(A))$
					\begin{enumerate}
		...
		\procedure{2.59}
			...
			\implementation
				\begin{enumerate}
					...
					\item Execute \procedurehr{2.20} on $G$ and let the tuple $\langle M,D,,N\rangle$ receive.
					...
					\begin{enumerate}
						...
						\item Execute \procedurehr{2.58} on $A$ and $u$.
					...
					\end{enumerate}
				\end{enumerate}
		\procedure[Euclidean division]{2.60}
			\objective
			...
			\implementation
				\begin{enumerate}
					...
					\begin{enumerate}
						...
						\item Execute \procedurehr{2.60} on the tuple $\langle e,b\rangle$. Let the tuple $\langle c,d\rangle$ receive the result.
			...
			\implementation
				\begin{enumerate}
					...
					\begin{enumerate}
						...
						\item Execute \procedurehr{2.61} on $s,q,k+1$ and let the tuple $\langle g_1,h_1\rangle$ receive.
						\item Verify that $s_{k+1}=g_1s_{m-1}+h_1s_m$.
						\item Verify that $\deg(g_1)=m-1-(k+1)=m-k-2$.
						\item Verify that $\deg(h_1)=m-2-(k+1)=m-k-3$.
						\item Execute \procedurehr{2.61} on $s,q,k+2$ and let the tuple $\langle g_2,h_2\rangle$ receive.
				...
				\end{enumerate}
		\procedure[Edwards' Sturm chain construction]{2.62}
			\objective
			...
			\implementation
				\begin{enumerate}
					\item Execute \procedurehr{2.59} on $A$ and let $\langle u,s_{t+1}\rangle$ receive the result.
					\item Verify that $us_t+s_{t+1}\sel_A=1$.
					\item Execute \procedurehr{2.60} on the tuple $\langle s_{t+1},s_t\rangle$. Let the tuple $\langle q_t,s_{t-1}\rangle$ receive the result.
					...
					\item Therefore using \procedurehr{2.58}, verify that $\frac{x^{t-1}\circ s_{t-1}}{x^t\circ s_t}=\tr(s_{t-1}(A)\sel_A(A))=\tr(I_{m,m})=m>0$.
					\item For $i=t-1$ down to $i=1$, do the following:
					\begin{enumerate}
						\item Execute \procedurehr{2.60} on the tuple $\langle -s_{i+1},-s_i\rangle$. Let the tuple $\langle q_i,s_{i-1}\rangle$ receive the result.
						...
						\item Therefore verify that $q_is_i-s_{i+1}=s_{i-1}$.
						\item Execute \procedurehr{2.61} on the tuple $\langle s,q,i-1\rangle$ and let $\langle p,j\rangle$ receive.
		...
		\procedure{2.63}
			...
			\implementation
				\begin{enumerate}
					\item Execute \procedurehr{2.62} on the matrix $A$ and let the tuple $\langle s,q\rangle$ receive the result.
					\item Execute \procedurehr{1.9} supplying the tuple $\langle s,q\rangle$. Let the tuple $\langle c,d\rangle$ receive the result.
		...
		\procedure{2.64}
			\objective
				Choose a symmetric $\mathcal{M}_{m,m}(\mathbb{Q})$, $A$. Let $t=\deg(\last_A)$. Execute \procedurehr{2.63} on $A$ and let the tuple $\langle c,d\rangle$ receive the result. Execute \procedurehr{2.20} on $A$ and let the tuple $\langle,,u,\rangle$ receive the result. The objective of the following instructions is to either show that $1=-1$ or to construct a list of non-negative integers $k$ such that $\sgn(u_{k_i}(c_i))=-\sgn(u_{k_i}(d_i))$ for $i=1$ to $i=t$.
			\implementation
				\begin{enumerate}
					...
					\begin{enumerate}
						...
						\begin{enumerate}
							...
							\item Cognizant of the execution of \procedurehr{1.8}, verify that $\sgn(s_t(c_i))=-\sgn(s_t(d_i))$.
		...
		\procedure{2.65}
			\objective
				Choose a symmetric $\mathcal{M}_{m,m}(\mathbb{Q})$, $A$. Execute \procedurehr{2.20} on $A$ and let the tuple $\langle,,u,\rangle$ receive the result. Execute \procedurehr{1.3} on $A$ and let $k$ receive. Let $t=\deg(\last_A)$. Let $n_j=\sum_{i=1}^t [k_i=j]$. The objective of the following instructions is to either show that $0<0$, or to show that $n_i=\deg(u_i)$ for $i=1$ to $i=m$.
			\implementation
				\begin{enumerate}
					...
					\begin{enumerate}
						\item Execute \procedurehr{1.3} on the polynomial $u_i$ along with $\deg(u_i)+1$ of the distinct pairs $\langle c_l,d_l\rangle$ such that $k_l=i$.
						\item \textbf{Abort procedure.}
					\end{enumerate}
		...
		\procedure[Upper triangular matrix multiplication]{2.66}
			\objective
		...
		\procedure{2.67}
			...
			\implementation
				\begin{enumerate}
					\item Using \procedurehr{2.10}, verify that $C_n(M_0A_0)=C_n(I_n)=1$.
					...
					\begin{enumerate}
						...
						\item Using \procedurehr{2.10}, verify that $C_n(N_i)=1\ne 0$.
		...
		\procedure{2.68}
			\objective
				Choose integers $m\ge n\ge 0$. Choose a $\mathcal{M}_{n,m}(\mathbb{Q}[x])$, $M$, and a $\mathcal{M}_{m,n}(\mathbb{Q}[x])$, $A_0$, such that $MA_0=I_n$. Execute \procedurehr{2.67} on $M$ and $A_0$ and let the tuple $\langle A_1,\cdots,A_n\rangle$ receive the result. The objective of the following instructions is to either show that $1=0$ or to show that $({A_i}^TA_i)_{[1:i+1],[1:i+1]}$ is a $\mathcal{D}_{i,i}(\mathbb{Q}[x])$ and ${A_i}^TA_i=\bdiag(({A_i}^TA_i)_{[1:i+1],[1:i+1]},({A_i}^TA_i)_{[i+1:n+1],[i+1:n+1]})$ for $i=1$ to $i=n$.
			\implementation
				\begin{enumerate}
					\item For $i=1$ to $i=n$, do the following:
					\begin{enumerate}
						...
						\item Now using \procedurehr{2.8}, verify that ${A_i}^TA_i$ and ${A_{i-1}}^TA_{i-1}$ are the same modulo the bottom-right $(n-i+1)\times(n-i+1)$ block.
			...
			\implementation
				\begin{enumerate}
					...
					\begin{enumerate}
						...
						\item Using \procedurehr{2.66}, for $j=1$ to $j=n$, verify that $({e_j}^TM)(A_ie_j)$
						\begin{enumerate}
							\item $={e_j}^T(MA_i)e_j$
					...
					\end{enumerate}
				\end{enumerate}
		\procedure[Cauchy-Schwarz inequality]{2.70}
			\objective
		...
		\procedure{2.71}
			...
			\implementation
				\begin{enumerate}
					\item Execute \procedurehr{2.67} on $M$ and $A_0$ and let the tuple $\langle A_0,A_1,\cdots,A_n\rangle$ receive the result.
					\item Let $i=n$.
					\item Verify that $\lVert A_i(x)e_j\rVert^2<\frac{1}{a^{(2i+2)!!}}$.
					\item Using \procedurehr{2.70}, verify that $({e_j}^TM(x)A_i(x)e_j)^2\le\lVert{e_j}^TM(x)\rVert^2\lVert A_i(x)e_j\rVert^2<\lVert M(x)\rVert^2\frac{1}{a^{(2i+2)!!}}\le a\frac{1}{a^{(2i+2)!!}}\le\frac{1}{a^{(2i)!!*2i}}\le 1$.
					...
					\end{enumerate}
					\item Otherwise, do the following:
					\item Using \procedurehr{2.69}, verify that $(1\lVert A_0e_1\rVert^2\cdots\lVert A_{\min(i,j-1)-1}e_{\min(i,j-1)}\rVert^2)^2=({e_j}^TM(x)A_i(x)e_j)^2<\frac{1}{a^{(2i)!!*2i}}\le 1$.
		...
		\procedure{2.72}
			\objective
				Choose a symmetric $\mathcal{M}_{m,m}(\mathbb{Q})$, $A$. Execute \procedurehr{2.64} on the matrix $A$ and let the tuple $\langle k\rangle$ receive the result. The objective of the following instructions is to either show that $0<0$ or to show that $\sum_{i=1}^t(m+1-k_i)=m$.
			\implementation
				\begin{enumerate}
					\item Execute \procedurehr{2.20} on the matrix $A$ and let the tuple $\langle,D,u,\rangle$.
					\item Using \procedurehr{2.65}, verify that $\sum_{i=1}^t(m+1-k_i)$
					\begin{enumerate}
		...
		\procedure{2.73}
			...
			\implementation
				\begin{enumerate}
					\item Execute \procedurehr{2.63} on the matrix $A$ and let the tuple $\langle c,d\rangle$ receive the result.
					\item Execute \procedurehr{2.4} with $xI_m-A$ as the choice matrix. Let the tuple $\langle M,D,,N\rangle$ receive the result.
				...
				\end{enumerate}
		\notation{2.32}
			Let us use the notation $\disc(A)$ to refer to the result yielded by executing \procedurehr{2.73} on the matrix $A$.
		\procedure{2.74}
			\objective
			...
			\implementation
				\begin{enumerate}
					...
					\item Execute \procedurehr{2.67} on the matrices $Q^TN^{-1}$ and $NQ$. Let the tuple $\langle ,,\cdots,,K\rangle$ receive the result.
					\item \textbf{Verify that $K$ is a $\mathcal{M}_{m,m+1-k}(\mathbb{Q}[x])$.}
					\item \textbf{Using \procedurehr{2.68}, verify that $K^TK$ is a $\mathcal{D}_{m+1-k,m+1-k}(\mathbb{Q}[x])$.}
					\item Let $E=Q^TN^{-1}K$.
					\item \textbf{Verify that $E$ is a $\mathcal{M}_{m+1-k,m+1-k}(\mathbb{Q}[x])$.}
					\item Execute \procedurehr{2.69} on the matrices $Q^TN^{-1}$ and $NQ$.
					\item \textbf{Now verify that $K=NQE$.}
					\item \textbf{Yield $\langle K,E\rangle$.}
				\end{enumerate}
		\procedure[Symmetric matrix spectral procedure initialization]{2.75}
			\objective
				Choose a symmetric $\mathcal{M}_{m,m}(\mathbb{Q})$, $A$. Choose a $\mathbb{Q}$ $\epsilon>0$. Execute \procedurehr{2.64} on the matrix $A$ and let the tuple $\langle k\rangle$ receive the result. The objective of the following instructions is to either show that $1<1$ or to construct $\mathbb{Q}$s, $0<\delta\le 1\le K'$, a list of $\mathcal{M}_{m,*}(\mathbb{Q})$s, $K$, and a list of $\mathbb{Q}$s, $g$, such that for $1\le i\le\lvert k\rvert$:
				\begin{enumerate}
			...
			\implementation
				\begin{enumerate}
					\item Execute \procedurehr{2.63} on the matrix $A$ and let the tuple $\langle c,d\rangle$ receive the result.
					\item Execute \procedurehr{2.20} with $xI_m-A$ as the choice matrix. Let the tuple $\langle M,D,u,N\rangle$ receive the result.
					...
					\item Execute \procedurehr{2.74} on $\langle k,m,N\rangle$ and let the tuple $\langle\langle K_1,E_1\rangle,\langle K_2,E_2\rangle,\cdots,\langle K_{\lvert k\rvert},E_{\lvert k\rvert}\rangle\rangle$ receive.
					\item \textbf{Using \procedurehr{2.72}, verify that $\sum_{p=1}^{\lvert k\rvert}\cols(K_p)=\sum_{p=1}^{\lvert k\rvert} m+1-k_p=m$.}
					...
					\begin{enumerate}
						...
						\item Execute \procedurehr{1.2} on the formal polynomial $u_{k_i}$, interval $(c_i, d_i)$, and target of $\frac{b}{U'}$. Let $\langle g_i\rangle$ receive the result.
						...
						\begin{enumerate}
							...
							\item Execute \procedurehr{2.71} with matrices $Q^TN^{-1}$ and $NQ$, rational number $g_i$, and column index $z$.
							\item \textbf{Abort procedure.}
						\end{enumerate}
						\item Otherwise, do the following:
						\begin{enumerate}
				...
				\end{enumerate}
		\procedure[Symmetric matrix spectral]{2.76}
			\objective
			...
			\implementation
				\begin{enumerate}
					\item Execute \procedurehr{2.75} on matrix $A$ and rational $\epsilon$. Let the tuple $\langle\delta,K',K,g\rangle$ receive the result.
					\item Let $C$ be a diagonal matrix whose $i^{th}$, where $1\le i\le t$, group of entries are $m+1-k_i$ $g_i$s.
					\item \textbf{Using \procedurehr{2.72}, verify that $C$ is $m\times m$.}
					\item Let $K$ be a matrix whose columns are the in-order concatenation of those of $K_1,K_2,\cdots,K_t$.
					\item \textbf{Using \procedurehr{2.72}, verify that $K$ is $m\times m$.}
