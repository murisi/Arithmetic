...
\begin{document}
	...
	\begin{multicols}{2}
		...
		\procedure{3.66}
			\objective
				Choose a rational number $X$ and a positive integer $K$. The objective of the following instructions is to construct rational numbers $a,b,c,d$, and a procedure, $p(x,n,m)$, to show that $\lVert x^K-1-\prod_r^{[0:K]}(x-\exp_n(\frac{r}{K}\tau_mi))\rVert^2\le(\frac{an}{m}+\frac{b}{n})^2$ when a complex number $x$ and positive integers $n,m$ such that $n>c$, $m>d$, and $\lVert x\rVert^2\le X$ are chosen.
			\implementation
				\begin{enumerate}
