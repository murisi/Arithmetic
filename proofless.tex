		...
		\subsection{Algorithm 44}\label{sec:algorithm 44}
			...
			\begin{enumerate}
				...
				\begin{enumerate}
					...
					\item \textbf{Abort algorithm.}
				\end{enumerate}
				\item Otherwise, verify the following in order:
				\begin{enumerate}
					\item All columns of $C_t(M^{-1})$ are non-zero.
					\item ${C_t(D)}_{1,1}\ne 0$.
					\item $C_t(N^{-1})\ne 0$
					\item \textbf{$C_t(F)=C_t(M^{-1}DN^{-1})=C_t(M^{-1})C_t(D)C_t(N^{-1})=C_t(M^{-1}){C_t(D)}_{1,1}e_1C_t(N^{-1})={C_t(D)}_{1,1}C_t(N^{-1})C_t(M^{-1})e_1\ne 0_{\binom{m^2}{t}\times 1}$}
				\end{enumerate}
			\end{enumerate}
		\subsection{Algorithm 45}\label{sec:algorithm 45}
			...
			\begin{enumerate}
				...
				\begin{enumerate}
					...
					\item Let $h=\pol(H)$.
					\item Execute \hyperref[sec:algorithm 5]{algorithm 5} on a $1\times 2$ matrix comprising $r$ followed by $h$. Let the tuple $\langle D,M,N\rangle$ receive the result.
					\item Execute \hyperref[sec:algorithm 6]{algorithm 6} on a $1\times 2$ matrix comprising $r$ followed by $h$. Let the tuple $\langle M^{-1},D,N^{-1}\rangle$ receive the result.
					\item Let $d=D_{1,1}$.
					...
					\begin{enumerate}
						...
						\item Verify that $\tr(u(A)h(A))=\tr(A^{z-1}b(A)c(A)d(A))=\tr(A^{z-1}c(A)b(A)d(A))=\tr(A^{z-1}c(A)f(A))=\tr(A^{z-1}c(A)0_{m\times m})=\tr(0_{m\times m})=0$.
						\item Therefore verify that 0=1.
						\item \textbf{Abort algorithm.}
					\end{enumerate}
					\item Otherwise, do the following:
					\begin{enumerate}
						...
						\item \textbf{Verify that $uf+s_{t+1}h=1$.}
					\end{enumerate}
				\end{enumerate}
			\end{enumerate}
