...
\begin{document}
    ...
    \subsection*{Comparing Procedural Mathematics}
        ...
        \begin{tabular}{|p{3.5cm}|p{3.5cm}|p{3.5cm}|p{3.5cm}|p{3.5cm}|p{3.5cm}|}
          \hline & \textbf{Classical Mathematics} & \textbf{Constructive Mathematics} & \textbf{Transparent Proofs / Proof Without Words} & \textbf{Procedural Mathematics} \\
          \hline \textbf{Mood} & Factual. Mathematicians mainly produce theorems, which are facts about abstract objects. & Same as classical mathematics. & None. & Intentional. Mathematicians produce procedures to achieve given objectives. But no claim is made that procedures achieve the objectives on every member of a domain. \\
          \hline \textbf{Generality} & Full. Universal quantifiers are used to express the generality of propositions over infinite domains. & Same as classical mathematics. & None becuase a theorem is proven for only one well-chosen case. That being said, most readers should be able to generalize proof to other cases. & Full. Symbols are used to indicate values that are only known at "run-time". At "run-time" these symbols take on a single value, rather than ranging over a set. \\
          \hline \textbf{Constructivity} & Optional. The existence of objects can be proven by showing that the lack of existence would be contradictory. & Required due to lack of law-of-excluded middle. & Required because a theorem is proven only on an explicitly selected object. & Required as there is no concept of existence. \\
          \hline \textbf{Correctness Criterion} & All inferences leading to theorem must be within stated and inference rules and must trace back to stated axioms. & Same as classical mathematics. & Almost the same as classical mathematics, but the generalization steps are not made explicit. & Adherence of procedure implementations to chosen mathematical semantic rules. Of course, it would be desirable to choose semantic rules that from experience have proven to yield "useful" procedures. \\
          \hline \textbf{Expressiveness} & Full. Sufficient for doing pure and applied mathematics and includes topics like transfinite set theory. & Subset of classical mathematics where all existential statements have witnesses. & Same as constructive mathematics because theorems are proven for explicitly chosen witnesses. & Subset of classical mathematics. Captures topics like trigonometry and calculus on complex variables. Fails to express topics like transfinite set theory. \\
          \hline \textbf{Imports} & Universal and existential quantification, transfinite set theory, inference rules, and axioms. & Same as classical mathematics. & Inference rules and axioms. Universal and existential quantification are made implicit. & Data structures, procedures, lambdas, recursion. \\
        \end{tabular}
    \subsection*{Methodology of Mathematical Experiment}
      Above I described a general method for doing mathematics but did not provide evidence that it is workable. The rest of this book intends to prove that this approach is indeed generally usable by reformulating the elementary parts of number theory, hard analysis, calculus, and linear algebra using the tools of procedural mathematics. So, while formal mathematics usually takes the format of definition-theorem-proof, this project has the format of declaration-procedure objective-procedure implementation. So where there usually would have been a statement and proof of Euler's totient theorem, \procedurehr{1.50} is provided, and where there would have been a definition of Euler's totient function, \declarationhr{1.16} is provided. Perhaps not surprisingly, software programming tools and concepts like lambdas, procedures, recursion, and modularity have turned out to be instrumental in rendering intelligable what could have been an indecipharable network of instructions/operations.
	\tableofcontents
