...
\begin{document}
			...
			\implementation
				\begin{enumerate}
					...
					\begin{enumerate}
						...
						\item Execute \procedurehr{2.11} on $s,q,k+1$ and let the tuple $\langle g_1,h_1\rangle$ receive.
						\item Verify that $s_{k+1}=g_1s_{m-1}+h_1s_m$.
						\item Verify that $\deg(g_1)=m-1-(k+1)=m-k-2$.
						\item Verify that $\deg(h_1)=m-2-(k+1)=m-k-3$.
						\item Execute \procedurehr{2.11} on $s,q,k+2$ and let the tuple $\langle g_2,h_2\rangle$ receive.
	...
	\section{Matrix Arithmetic}
		...
		\procedure{3.71}
			...
			\implementation
				\begin{enumerate}
					...
					\item Execute \procedurehr{2.10} on the tuple $\langle s_{t+1},s_t\rangle$. Let the tuple $\langle q_t,s_{t-1}\rangle$ receive the result.
					...
					\begin{enumerate}
						\item Execute \procedurehr{2.10} on the tuple $\langle -s_{i+1},-s_i\rangle$. Let the tuple $\langle q_i,s_{i-1}\rangle$ receive the result.
						...
						\item Therefore verify that $q_is_i-s_{i+1}=s_{i-1}$.
						\item Execute \procedurehr{2.11} on the tuple $\langle s,q,i-1\rangle$ and let $\langle p,j\rangle$ receive.
		...
		\procedure{3.72}
			...
			\implementation
				\begin{enumerate}
					\item Execute \procedurehr{3.71} on the matrix $A$ and let the tuple $\langle s,q\rangle$ receive the result.
			...
			\implementation
				\begin{enumerate}
					...
					\begin{enumerate}
						...
						\item Using \procedurehr{3.75}, for $j$ in $[0:n]$, verify that $({e_j}^TM)(A_ie_j)$
						\begin{enumerate}
							\item $={e_j}^T(MA_i)e_j$
		...
		\procedure{3.79}
			\objective
				Choose a symmetric $\mathcal{M}_{m,m}(\mathbb{Q})$, $A$. Let $t=\deg(\last_A)$. Execute \procedurehr{3.73} on the matrix $A$ and let the tuple $\langle k\rangle$ receive the result. The objective of the following instructions is to either show that $0<0$ or to show that $\sum_{i=0}^t(m-k_i)=m$.
			\implementation
				\begin{enumerate}
					\item Execute \procedurehr{3.28} on the matrix $A$ and let the tuple $\langle,D,u,\rangle$.
					\item Using \procedurehr{3.74}, verify that $\sum_{i=0}^t(m-k_i)$
					\begin{enumerate}
		...
		\procedure{3.80}
			...
			\implementation
				\begin{enumerate}
					\item Execute \procedurehr{3.72} on the matrix $A$ and let the tuple $\langle c,d\rangle$ receive the result.
				...
				\end{enumerate}
		\notation{3.30}
			Let us use the notation $\disc(A)$ to refer to the result yielded by executing \procedurehr{3.80} on the matrix $A$.
		\procedure{3.81}
			\objective
			...
			\implementation
				\begin{enumerate}
					...
					\item Execute \procedurehr{3.76} on the matrices $Q^T{N^{-1}}_*$ and $N_*Q$ and let the tuple $\langle Z\rangle$ receive.
					\item Let $K=Z_{m-k}$.
		...
		\procedure{3.82}
			...
			\implementation
				\begin{enumerate}
					...
					\item Execute \procedurehr{3.81} on $\langle k,m,N\rangle$ and let the tuple $\langle\langle K_0,E_0\rangle,\langle K_1,E_1\rangle,\cdots,\langle K_{\lvert k\rvert-1},E_{\lvert k\rvert-1}\rangle\rangle$ receive.
					\item \textbf{Using \procedurehr{3.79}, verify that $\sum_{p=0}^{\lvert k\rvert}\cols(K_p)=\sum_{p=0}^{\lvert k\rvert} m-k_p=m$.}
		...
		\procedure{3.83}
			...
			\implementation
				\begin{enumerate}
					\item Execute \procedurehr{3.82} on matrix $A$ and rational $\epsilon$. Let the tuple $\langle\delta,K',K,g\rangle$ receive the result.
					\item Let $C$ be a diagonal matrix whose $i^{th}$, where $0\le i<t$, group of entries are $m-k_i$ $g_i$s.
					\item \textbf{Using \procedurehr{3.79}, verify that $C$ is $m\times m$.}
					\item Let $K$ be a matrix whose columns are the in-order concatenation of those of $K_0,K_1,\cdots,K_{t-1}$.
					\item \textbf{Using \procedurehr{3.79}, verify that $K$ is $m\times m$.}
